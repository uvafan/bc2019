...
export class CombatUnit extends Robot{
    constructor(rc){
        ...
        this.protectingChurch = false;
        this.getFirstTarget();
        this.prelimTrusted=[];
        this.trusted=[];
        this.exploreDir = -1;
        this.lastHealth = SPECS.UNITS[this.me.unit].STARTING_HP;
        this.enemyLoc = [0,0];
    }
    ...
    getFirstTarget(){
        ...
        for(var i=0;i<visRobots.length;i++){
            ...
            if(r.team==this.me.team&&this.distBtwnP(r.x,r.y,this.me.x,this.me.y)<=2&&(r.unit==SPECS['CASTLE']||r.unit==SPECS['CHURCH'])){
                ...
                this.enemyLoc = this.reflect(r.x,r.y);
                this.enemyDists = this.runBFS(this.enemyLoc,false);
                if(locb&(1<<12)){
                    ...
                    if(locb&(1<<13)){
                        ...
                        var loc = this.getLocFromBroadcast(locb^(1<<12));
                        target = this.stepTowards(loc,this.enemyLoc,0);
                        this.protectingChurch=true;
                    }
    ...
    stepTowards(loc,to,steps){
        ...
        while(q.length>0){
            ...
            for(var i=0;i<8;i++){
                ...
                if(d<bestDist&&((nx+ny)%2==1)&&!(this.rc.karbonite_map[ny][nx]||this.rc.fuel_map[ny][nx])){
                    bestDist=d;
                    best=[nx,ny];
                }
    ...
    turn(rc){
        ...
        else if(this.me.health<this.lastHealth){
            this.lastHealth = this.me.health;
            return this.navTo(this.enemyDists,this.enemyLoc,params.ATT_NAV_WEIGHTS,true,true);
        }
        //this.log('mh '+this.me.health+'lh '+this.lastHealth);
        this.lastHealth = this.me.health;
        if(!this.target)
            return null;
        ...
        var nav_weights = ((this.attacking||this.protectingChurch)?params.ATT_NAV_WEIGHTS:params.DEF_NAV_WEIGHTS);
    ...
    pickBetterTarget(){
        ...
        for(var i = 0; i < dist.length; i++){
            var nx = this.me.x+dist[i][0];
            var ny = this.me.y+dist[i][1];
            if(!this.offMap(nx,ny) && !this.rc.karbonite_map[ny][nx] && !this.rc.fuel_map[ny][nx] && this.isWalkable(nx,ny)){
                ...
                if(nx%2 == 0 && ny %2 == 0){
                    ...
                    for(var j = 0; j < moves.length; j++){
                        ...
                        if(!this.isPassable(nnx,nny) || this.rc.karbonite_map[nny][nnx] || this.rc.fuel_map[nny][nnx])
                            score++;
