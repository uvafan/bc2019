...
export class Pilgrim extends Robot{
    ...
    updateChurchLoc(){
        var options = this.getDxDyWithin(1,16);
        for(var i=0;i<options.length;i++){
            var nx = this.target[0]+options[i][0]
            var ny = this.target[1]+options[i][1]
            if(this.isWalkable(nx,ny)&&this.isSafe(nx,ny)&&!(this.rc.karbonite_map[ny][nx]||
                this.rc.fuel_map[ny][nx])){
                this.updateTarget([nx,ny]);
                this.updated=true;
                break;
            }
        }
    ...
    buildChurch(){
        ...
        for(var i=0;i<this.adjDiagMoves.length;i++){
            ...
            var d=this.distBtwnP(nx,ny,this.target[0],this.target[1]);
            var safe = this.isSafe(nx,ny);
            var score = (safe?100:0)-d;
            if(this.isWalkable(nx,ny)&&score>bestScore&&!this.rc.karbonite_map[ny][nx]&&!this.rc.fuel_map[ny][nx]){
