...
export class Castle extends Structure{
    ...
    turn(rc){
        ...
        if((this.unitCounts[SPECS['CASTLE']]<this.otherCastleLocs.length+1||this.rc.health<SPECS.UNITS[SPECS['CASTLE']]['STARTING_HP'])
            && this.me.turn>=params.ATTACK_NOW-2 && this.me.turn-params.ATTACK_NOW<this.enemyCastleLocs.length){
            var signal=params.RANDOM_ONE;
            if(this.me.turn+1==params.ATTACK_NOW)
                signal=params.RANDOM_TWO;
            else if(this.me.turn>=params.ATTACK_NOW){
