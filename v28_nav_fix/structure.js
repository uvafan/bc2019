...
export class Structure extends Unit{
    ...
    shouldBuild(priority,unit){
        ...
        var thresh = params.SAVE_EXP_THRESH;
        if(priority<thresh){
            fuelSave+=(thresh-priority)*(thresh-priority)*(thresh-priority)*params.EXTRA_FUEL_SAVINGS;
            karbSave+=(thresh-priority)*(thresh-priority)*(thresh-priority)*params.EXTRA_KARB_SAVINGS;
        }
        if(this.makeStuff){
